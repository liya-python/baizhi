...
class Course(BaseModel):
    ...
    @property
    def brief_html(self):
        # src="/media/2020/09/29/baeodk.png"
        brief = self.brief.replace('src="/media', 'src="%s/media' % "http://127.0.0.1:9000/")
        return brief
    ...
    # def expire_end_time(self,expire_id):
    #     if expire_id >0:
    #         expire_time = self.course
    #         print(expire_time)
    #         return expire_time
        ...
        if len(active_list) > 0:
            ...
            if self.price >= condition:
                ...
                if sale == "":
                    ...
                    # 满减 满500-80 满400-40 满300-20 满200-10
                    sale_split = sale.split("\r\n")
                    # 把当前价格满足的优惠都放入列表
                    price_list = []
                    for sale_item in sale_split:
                        item = sale_item[1:]
                        con_price, sale_price = item.split("-")
                        if self.price >= float(con_price):
                            price_list.append(float(sale_price))
                        if len(price_list) > 0:
                            price = self.price - max(price_list)

        return "%.2f" % price

    def expire_real_price(self,expire_id=0):
        """
        点击有效期对应后的价格
        1. 先判断当前的有效期是否大于0  大于0处理有效期的价格
        2. 先根据有效期获取到有效期对应的价格
        3. 根据有效期的价格来进行活动优惠的计算
        4. 根据不同的活动规则计算优惠后的价格即可
        :return:
        """
    ...
    def discount_end_time(self):
        active_list = self.active_list()
        # print(active_list)
        if len(active_list) > 0:
            active = active_list[0]
            # print(active)
            end_time = active.active.end_time
            print(end_time)
            return  end_time
